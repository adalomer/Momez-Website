					...
					<div className="bg-white dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700 p-6">
								...
								<div key={item.id} className="flex items-center justify-between p-4 bg-slate-50 dark:bg-slate-900/50 rounded-lg">
									...
									<div className="text-right">
										...
										<p className="text-sm text-slate-500 dark:text-slate-400">
											{formatPrice(item.price, language as Language)} / {t('order.perItem')}
										</p>
									</div>
								</div>
							))}
						...
						<div className="mt-6 pt-6 border-t border-slate-200 dark:border-slate-700 space-y-2">
							<div className="flex justify-between text-slate-600 dark:text-slate-400">
								...
								<span>{formatPrice(order.subtotal || 0, language as Language)}</span>
							</div>
							<div className="flex justify-between text-slate-600 dark:text-slate-400">
								<span>{t('order.shipping')}</span>
								<span>{formatPrice(order.shipping_cost || 0, language as Language)}</span>
							</div>
							<div className="flex justify-between text-lg font-bold text-slate-900 dark:text-white pt-2 border-t border-slate-200 dark:border-slate-700">
								...
								<span>{formatPrice(order.total || 0, language as Language)}</span>
							</div>
						</div>
					</div>
					...
					<div className="bg-white dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700 p-6">
						...
						<div className="space-y-3">
							<div>
								...
								<span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium mt-1 ${statusColors[order.status as keyof typeof statusColors] || statusColors.pending
									}`}>
