						...
						<div className="space-y-6 max-h-[70vh] overflow-y-auto pr-2">
							...
							<div className="border border-slate-200 dark:border-slate-700 rounded-lg p-4">
								...
								{editForm.colors.length === 0 ? (
									<div className="text-center py-6 text-slate-500"><Palette className="w-10 h-10 mx-auto mb-2 opacity-50" /><p>No colors added yet</p></div>
								) : (
										...
										{editForm.colors.map(color => (
											<div key={color.id} className="border border-slate-200 dark:border-slate-700 rounded-lg p-3">
												<div className="flex items-center justify-between mb-3">
													...
													<div className="flex items-center gap-2">
														{!color.isDefault && <button type="button" onClick={() => setDefaultColor(color.id)} className="text-xs text-slate-500 hover:text-primary">Make default</button>}
												...
												<div className="grid grid-cols-4 sm:grid-cols-6 gap-2">
													...
													<label className="aspect-square rounded border-2 border-dashed border-slate-300 dark:border-slate-600 hover:border-primary flex flex-col items-center justify-center text-slate-400 hover:text-primary cursor-pointer">
														...
														<Upload className="h-4 w-4" /><span className="text-[10px] mt-1">{uploadingImage === color.id ? '...' : 'Add'}</span>
													</label>
												</div>
											</div>
										))}
