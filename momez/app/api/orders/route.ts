...
export async function POST(request: NextRequest) {
  try {
    ...
    // Toplam hesapla (compare_at_price varsa indirimli demektir, asıl fiyat price'dır)
    ...
    for (const item of cartItems) {
      ...
      await db.query(
        `INSERT INTO order_items 
          (id, order_id, product_id, product_name, size, quantity, unit_price, total_price) 
         VALUES (?, ?, ?, ?, ?, ?, ?, ?)`,
        [itemId, orderId, item.product_id, item.product_name, item.size, item.quantity, price, price * item.quantity]
      )
