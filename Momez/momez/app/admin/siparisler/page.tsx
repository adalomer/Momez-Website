									...
									<tr key={order.id} className="hover:bg-gradient-to-r hover:from-slate-50 hover:to-transparent dark:hover:from-slate-900/50 dark:hover:to-transparent transition-all duration-200">
										...
										<td className="px-6 py-4 whitespace-nowrap">
											<span className="text-lg font-bold bg-gradient-to-r from-green-600 to-emerald-600 bg-clip-text text-transparent">
												{formatPrice(Number(order.total) || 0, language as Language)}
											</span>
										</td>
										<td className="px-6 py-4 whitespace-nowrap">
											<span className={`inline-flex items-center px-3 py-1.5 rounded-full text-xs font-bold shadow-sm ${statusColors[order.status as keyof typeof statusColors] || statusColors.pending
												}`}>
										...
										<td className="px-6 py-4 whitespace-nowrap">
											<div className="flex items-center gap-2">
												...
												<select
													...
													className="text-sm border-2 border-slate-300 dark:border-slate-600 rounded-xl px-4 py-2.5 bg-white dark:bg-slate-800 text-slate-900 dark:text-white focus:outline-none focus:ring-4 focus:ring-primary/20 focus:border-primary cursor-pointer transition-all hover:border-primary hover:shadow-lg font-semibold hover:scale-105 transform duration-200 [&>option]:bg-white [&>option]:dark:bg-slate-800 [&>option]:text-slate-900 [&>option]:dark:text-white"
												>
